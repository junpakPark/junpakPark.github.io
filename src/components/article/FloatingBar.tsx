...
const Button: React.FC<ButtonProps> = ({onClick, path, ariaLabel}) => (
    <button
        onClick={onClick}
        className="w-8 h-8 inline-flex justify-center items-center rounded-full text-gray-400 hover:text-gray-700 hover:bg-gray-100"
...
const ScrollButtons: React.FC = () => {
    ...
    return (
        <div className="flex justify-around ml-4 border border-solid rounded-lg">
            {scrollConfigs.map((config, index) => (
                <Button key={index} {...config} />
            ))}
        </div>
    );
